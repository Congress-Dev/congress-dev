        ...
        <>
            <Callout>
                <h4 className="appropriation-title" onClick={handleClick}>
                    {appropriation.parentId ? "Sub " : ""}Appropriation #{appropriation.appropriationId} {appropriation.newSpending && <Tag intent="warning">New Spending</Tag>}
                </h4>
...
const AppropriationTree = ({ appropriations, onNavigate }) => {
    function createNestedAppropriationTree(appropriations) {
        ...
        lodash.forEach(rootApprops, (rootApprop) => {
            ...
            results.push(
                <AppropriationItem key={rootApprop.parentId} appropriation={rootApprop} onNavigate={onNavigate} />
            );
            if (children) {
                ...
                results.push(
                    <div className="nested-appropriation" style={{ paddingLeft: '25px' }}>
                        {children.map((child) => (
                            <AppropriationItem key={child.parentId} appropriation={child} onNavigate={onNavigate} />
                        ))}
                    </div>
                );
            }
    ...
    return (
        createNestedAppropriationTree(appropriations)
    );
};
